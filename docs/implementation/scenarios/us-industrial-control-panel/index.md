...
- This scenario assumes a US-only market panel. For EU or global markets, see [Scenario 02]({{ '/implementation/scenarios/global-machine/' | relative_url }}).
- Safety function design (PL/SIL) is not the primary focus of this scenario. If safety functions are required, add ISO 13849-1 and see [Scenario 04]({{ '/implementation/scenarios/networked-safety-plc/' | relative_url }}).

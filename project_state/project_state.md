...
**Last Updated:** 2026-07-17 (AI-integration expansion — Slice G Worked Architectures shipped; **7-page build COMPLETE**; all 50.x merged)
**Status:** Active
**Current Delivery:** **Phase 49b COMPLETE (2026-07-13).** The project-hardening gate and the corpus-owned, 40-row AI/ML method register are implemented. Generated data lives under `docs/_data/ai_methods/`; the Review-pending page is `/design/ai-integration/`. Every row carries the full decision/safety schema. No learned method receives level 5 or a safety function, and chemical/biological authority remains Planned.
**Next Gate:** **Phase 49c is not authorised.** Close and adversarially review the chemical/biological evidence gaps and obtain explicit owner authorisation before layer-design pages. Remaining accepted architecture recommendations can continue as separately bounded hardening slices.
**Current Phase:** **Phase 49a COMPLETE (2026-07-13) — AI/ML source-closure sprint (research only; no site or corpus content).** Multi-agent workflow: 6 research workstreams under publisher-verification rules + an adversarial stage attacking every proposed authority ceiling. **98 claims (72 publisher-verified); 80 ceilings attacked, 76 refuted** — every refutation pushed the ceiling *down*. Outputs (work tier): `evidence-table.md`, `authority-ceilings.md`, `adversarial-verdicts.md`, `49a-findings.md`, extended `source-register.md`. **Governing finding:** ISO/IEC TR 5469:2024 is real and publisher-verified but is a **Technical Report — guidance, not certifiable requirements**; its requirement-bearing successor ISO/IEC TS 22440 is still a committee draft, so **no published normative standard exists against which a learned closed-loop safety function could be certified**. The law leads the standards: EU MR 2023/1230 Annex I Part A items 5–6 (ML self-evolving safety components → mandatory notified body) and Annex III EHSR 1.2.1 (statutory authority ceiling: defined task/movement space, always correctable, no hazardous self-modification *including during learning*); AI Act Art. 6(1) binds ~6 months later. **Convergent architecture across standards, law, and every documented deployment: the learned policy holds operational authority inside an envelope; a verified non-learned layer holds the safety function and the veto.** Sprint caught **three orchestrator errors** — the "kHz cannot pass through OPC UA" premise is **false as a protocol claim** (PubSub/TSN measured at 250 µs) and must be rewritten around four different constraints; the standard is **ISO/IEC** TR 5469, not IEC; and Paderborn *does* carry vibration + motor current synchronously (vibration beat current in every case). Also **confirmed Phase 45's 20 Jan 2027 MR date** — correct only via the corrigendum. **Go/no-go: GO** for core AI/ML, interfaces (with the mandatory kHz rewrite), classical baselines; **PARTIAL** for chemical/biological — structure and reality-check only, **no authority claims** (their ceilings got zero adversarial coverage; the run hit a session limit). Details in `change_log.md` and `49a-findings.md`.
**Previous Phase:** **Phase 48 COMPLETE (2026-07-12) — PLC/IPC hardware reference + vendor documentation index.** Two data-driven pages under `/tools/manufacturers/`, both at **Review pending**: **Official Vendor Documentation Index** (`vendor_doc_links.yml`, 129 official-manual links + 8 master portals across Siemens / Allen-Bradley / Beckhoff; `utm_source` stripped; liveness-checked 2026-07-12 — 101/129 return 200/206 and **all 28 failures are bot-gated Siemens portals**, not dead links, which is why the **publication ID is carried as the stable reference**) and **PLC & IPC Hardware Families** (`plc_hardware_families.yml`, 38 families; the three architectural models, a Mermaid selection flow driven by application requirement rather than brand, cross-vendor size mapping labelled as *role* equivalence not performance, Siemens suffix decoding, an "industrial PCs are not controllers" section, 7 numbered mistakes). **Every lifecycle/market claim carries an `as_of: July 2026` stamp and a verify-current warning** — the CompactLogix 5390 is recorded as *announced* (vendor availability Q4 2026), not shipping. Source positioning language softened per CONTENT_STANDARDS §6. Sources relocated out of `temp/`: md → `control-standards/work/general/plc_ipc_hardware_intake_2026-07.md`, xlsx → `planning/`. Both pages wired into nav + section index. Gates: Jekyll build clean (2.074 s), zero broken internal links (370 files), `validate_ai_boundaries.py` at the pre-existing 2-failure baseline. Details in `change_log.md`.
**Previous Phase:** **Phase 47 COMPLETE (2026-07-12) — PLC software expansion.** The `/fundamentals/plc-software/` section doubled from 4 to 8 pages, corpus-first per CONTENT_STANDARDS §1: four deep corpus notes in `control-standards/rag/training_modules/plc_software/` (ladder_logic 509 ln · algorithms_equipment_staging 482 ln · packml_isa88_isa95 459 ln · vendor_architectures 515 ln) with matching site distillations — **Ladder Logic Fundamentals · PLC Algorithms & Equipment Staging · PackML, ISA-88 & ISA-95 · Vendor Programming Architectures** — all at **Review pending**. Source discipline per roadmap: `temp/plc_software.md` (4,976 ln) relocated to `control-standards/work/general/plc_software_intake_2026-07.md` with all 28 `utm_source` params stripped; all 31 citations liveness-checked (24 × 200/206; the 7 failures are bot-gated portals — Siemens Industry Support, ISA blog, SEMI — not dead links); one source citation mis-attribution corrected (the Logix tasks/programs/routines manual is **1756-PM005** — the source paired the claim with the 1756-PM006/SFC URL); paraphrase-only throughout; the source's market-position language ("strongest", "dominant") reframed to neutral fit language per §6. Nav + section index carry all 8 pages in learning order; RAG mirror regenerated (314 files, 4 new). Gates: Jekyll build clean (1.656 s), zero broken internal links (368 files), `validate_ai_boundaries.py` at the pre-existing 2-failure baseline. Details in `change_log.md`.
**Earlier Phase:** **Phase 45 COMPLETE (2026-07-12) — standards accuracy pass.** Claim-by-claim triage of the third external review, with **every edition claim verified against the official publisher** (IEC webstore, ISO OBP, NFPA, UL, EUR-Lex) before adoption; anything the publisher's free text could not settle is flagged on the page rather than asserted. Fixed the safety-significant errors: the **ISO 12100 → PLr misattribution** (the S/F/P risk graph that yields PLr is **ISO 13849-1 Annex A**; ISO 12100 supplies the risk-assessment methodology; IEC 62061 has its own SIL method); **ISO 13849-1 and IEC 62061 are alternative routes, not a ladder** (removed the false "IEC 62061 is required above SIL 2" rule); **IEC 60204-1** edition (**2016+AMD1:2021** — no 2018 edition exists), its **fabricated** "25 V AC / 60 V DC" lower scope threshold (the official scope has no lower bound), its true **18-clause** structure (Clause 18 Verification was absent), and **emergency stop restricted to Category 0 or 1**; **CE-marking** legal wording (harmonised standards are voluntary, conferring presumption of conformity) plus the **20 Jan 2027** Machinery Regulation transition (per the corrigendum — the original OJ text says 14 Jan); **IEC 62443** part editions (2-1:2024, 2-4:2023), zones/conduits and SL-T reassigned to **62443-3-2**, and removal of the default-SL prescriptions (SL-T must be risk-derived); **IEC 61511 spans SIL 1–4** — both the site *and the external review itself* wrongly claimed SIL 1–3; **UL 508A** is **3rd Ed. (2018-04-24), rev. through 2025-06-26** (there is no "UL 508A:2022"), and UL listing is **not** universally required; **NEC** three-field edition metadata (covered / latest published / AHJ-enforceable). Swept every internal status label out of the presentation layer using the governed 5-term vocabulary — **the review's proposed replacement vocabulary was REJECTED** (conflicts with CONTENT_STANDARDS §3). **Found independently, not in the review:** NFPA 79's scope voltage was stale site-wide (600 V → **1000 V**); the **IEC 60204-1 corpus module had been drafted against the superseded 2005 edition** (Ed. 5.0 clause title, a 9.2.5.4.2 citation that does not exist in Ed. 6), **skipped Clauses 12 and 13 entirely**, merged 16/17, and mis-numbered everything from 12 onward — module renumbered against the official contents with ~600 cross-references rewritten and Clauses 12/13/17 created; and **22 corpus files ended with an AI drafting artifact** ("Would you like me to move on to Clause 15?"), all stripped. Pages an agent touched are now at **Review pending** (an agent never marks its own work Reviewed). **IEC 62061 is at Needs revalidation** — the current reference is 2021+AMD1:2024+**AMD2:2026** (CSV Ed. 2.2, published 2026-03-20); the page must be rebuilt from the consolidated text, not patched (SILCL → subsystem *maximum SIL*; scope extended beyond E/E/PE).
**Next Phase:** **Phase 50 — whole-project hardening & hygiene (recorded 2026-07-16).** Plan: `planning/2026-07-16-phase50-hardening-hygiene-plan.md`, from a four-track audit (toolkit, site, corpus/governance, repo hygiene) reconciled against the accepted 2026-07-13 recommendations and the 2026-07-16 live AI/ML-page review. **Verified done:** P0.1 (`YL-301` direction fix), P0.2 (UL 508A provenance), P0.3 (mirror), release-check module + CI (`--profile full` on push and PR); all gates green 2026-07-16 (155 tests, 10 doctests, clean build, 0 broken links, 374/374 boundaries, mirror byte-identical). **Phase 50 workstreams, in priority order:** (A) corpus correctness — fix the still-unfixed NFPA 79 Ch06 "Chapter 7" numbering bug, the suspect Ch15 "Table 7.2.7" citation, 3 empty placeholders (NFPA79 Ch04/Ch17, IEC 60204-1 Cl05), 3 conversational AI artifacts (2 reference_models + Cl18); add a corpus-quality validator (these defect classes have no automated check — root cause of their survival); document the undocumented `CONTENT_CLASS`/`STATUS` header schema in governance and drive metadata debt to zero; reconcile `_index.yaml` (iec_62443 + offshore missing from inventory; one case-mismatched reference that breaks on case-sensitive filesystems). (B) toolkit — the not-started accepted slices: validated-I/O seam (duplicate tags still silently overwrite loop sheets), licensed-table validation + stdlib-only core install (pymupdf/pypdf still unconditional deps), thin CLI + tests (486-line `cli.py` and `cite.py` are the only untested modules). (C) hygiene — relocate tracked `temp/` intakes per governance, delete `main.py`, untrack stray root binaries, prune 34 stale local branches, split this file into current-state + `project_state/history/`, fix environment.md drift (Python 3.13 pin vs 3.12.7 actual; stale test counts). (D) site editorial — `review:` rollout for vendor/ (58) + industries/ (34) pages with an agreed exemption list (283/360 pages lack blocks), rebuild IEC 62061 from consolidated Ed 2.2 (only Needs-revalidation page), confirm 6 duplicate nav URLs, re-run vendor-link liveness (28/129 dead, all bot-gated Siemens). (E) AI/ML decision-support upgrade — correct ISO/IEC TR 5469 identity and unsafe universal-fallback wording; clarify the level-4 architectural ceiling; add a canonical-data-driven method selector, promotion-gate worksheet, versioned result contract, row-level evidence provenance, and scoped machinery/AI checklist. Phase 49c remains not authorised; chemical/biological rows stay `Planned`.

**Phase 50 progress:** `/design/ai-integration/` visualization pass merged and deployed 2026-07-16 (4 Mermaid diagrams, veto-gate + result-contract code, per-family tables with anchors, legend, perception callout; deploy run green, live page verified). Second slice (merged/deployed): plain-terms orientation, nine-term glossary, pump scenario at every authority level, entry-reading key, plain-terms section leads. Third slice (merged/deployed): `example` field on all 42 canonical register rows (generator-enforced + test) with per-entry rendering and a topic-appropriate context panel. A post-deploy review is recorded as Workstream E (50.12–50.15): correctness/safety language, interactive method selection, deployment gates and result-contract hardening, then evidence provenance/conformity guidance; no Phase 49c authority work is included. **Workstream A started — 50.1 corpus correctness complete** on `fix/phase50-corpus-correctness`: 5 conversational artifacts removed/neutralized, NFPA 79 Ch06 "Chapter 7" numbering fixed, Ch15 suspect citation flagged, 3 empty placeholders gap-flagged (not fabricated — licensed values transcribed later), iec_60204_1 README stub replaced; mirror regenerated; full gate green. **50.2 corpus-quality validator complete** on `feat/phase50-corpus-quality-validator`: new `tools/validate_corpus_quality.py` (hard-fails conversational artifacts + empty placeholders, wired into corpus/full profiles, 4 tests); corpus metadata debt driven to zero (37 files backfilled CONTENT_CLASS/STATUS) with the ratchet baselines dropped 25/37 → 0/0 so regressions hard-fail; header schema documented in governance/AI_WORKFLOW.md §1. Full gate green; RAG metadata warnings eliminated. **50.12 AI/ML correctness/safety pass complete** (stacked on the same branch): fixed the live `IEC TR 5469` → `ISO/IEC TR 5469` source-identity error, replaced the unsafe universal `baseline.compute` veto fallback with a hazard-analysis-derived `on_fail` policy, and clarified level 4 as an architectural ceiling (no learned row exceeds level 3). Full gate green. **50.3 `_index.yaml` reconciliation complete** (stacked on the same branch): `check_index()` added to `tools/validate_corpus_quality.py` (TDD, 5 new tests — 9 total) verifying the master index against disk: every indexed `folder:` exists, every corpus standards folder under `us/`/`international/` is in the inventory, and every `folder:`/`file:`/`guidance_file:` reference resolves **case-sensitively** (directory-listing comparison, so macOS's case-insensitive filesystem can't mask breakage on case-sensitive CI). Run against the real corpus it caught exactly the planned defects — then the data was fixed: iec_62443 (4 files) and offshore (2 files) added to the `standards:` inventory; the 6 lowercase `software_safety_and_intrinsic_safety_standards.md` references corrected to the on-disk `Software_Safety_...` casing; `15-Standard Minimum Compliance Stack.md` + `standards_atlas_diagrams_reference.md` added to `guidance_documents`; the stale `edition:` fields corrected to Phase 45's publisher-verified values (UL 508A "2022" → "3rd Ed. (2018), rev. 2025-06-26"; IEC 60204-1 "2018" → "2016+AMD1:2021"); `crosswalks/overlap_notes/GENERATION_STATUS.md` re-stamped as **paused/backlog, 3 of 28 notes exist** (writing the 25 stays backlog per plan). Mirror regenerated; full gate green. **The stacked branch `feat/phase50-corpus-quality-validator` (50.2 → Workstream-E plan → 50.12 → 50.3) was ff-merged to `master` and deployed 2026-07-17** — full gate green, Pages deploy run `29595960732` green (build + deploy), live `/design/ai-integration/` verified carrying the `ISO/IEC TR 5469` fix, merged branch deleted. **50.4 validated-I/O seam merged to `master`** 2026-07-17: `IOListError` + `IOList.raise_for_problems()` gate every generator (bom, wire schedule, loop sheets, legend, FAT/SAT, tags-from-io/Modbus, design package) so malformed or colliding I/O is rejected before any artifact is produced — closing the silent duplicate-tag overwrite in `generate_loop_sheets`. 10 new TDD tests (`test_io_validation_seam.py`), 175 passing, full gate green; branch deleted. **50.13 classification vocabulary drafted for owner review** on `docs/phase50-13-classification-vocabulary`: `planning/2026-07-17-phase50-13-classification-vocabulary.md` proposes the four new canonical fields the interactive selector needs (`method_class`, `decision_type`, `data_availability`, `safety_relevance`) with inclusion tests, four generator-enforced invariants (chief: **safety-related ⟹ deterministic**), a full 42-row assignment table, and five flagged judgement calls. No canonical/generator/site data changed — **50.13 implementation gated on approval of this draft**. **50.5 licensed-table module + packaging cleanup complete** on `feat/phase50-licensed-tables` (off master): loader now hard-rejects missing/partial provenance, non-list/non-object `data`, and (for schema-mapped tables) rows missing a schema-`required` field — all with file-naming errors; the wheel force-includes samples+schemas as `cst/_bundled_tables/` and `load_table` resolves `CST_TABLES_DIR` → repo checkout → bundled, verified by installing the built wheel in a clean venv and loading a sample from `/tmp`; `pymupdf`/`pypdf` moved to an optional `fe-study` extra so the core install is dependency-free. 12 loader tests, 180 total, full gate green. Awaiting owner merge. **AI-integration 7-page expansion (owner-authorised 2026-07-17)** — plan `planning/2026-07-17-ai-integration-7page-build-plan.md`. The single register page is being split into the authority-first structure from the Phase 49 presentation plan, corpus-first, one bounded slice at a time. **Slice A shipped** (register split to `/method-register/`; index is the authority gate). **Slice B shipped** on `feat/phase51-ai-integration-safety-boundaries`: corpus note `safety_boundary.md` + site page `/design/ai-integration/safety-boundaries/` (why a learned component cannot hold a safety function today — deterministic FS standards, TR 5469 guidance-only, TS 22440 unpublished, EU MR/AI Act statutory ceiling, NIST SP 800-82r3 OT placement; original standards-vs-law diagram). **Source gap closed:** IEC 62061 verified at the IEC webstore (Ed. 2.0, no AI/ML in scope); TS 22440 confirmed Committee Draft (Feb 2026, 3 parts). **Slice C shipped** on `feat/phase51-ai-integration-interfaces`: corpus note `interfaces_edge.md` + site page `/design/ai-integration/interfaces/` (four edge-inference constraints, OPC UA vs Sparkplug capability table, Observer-role write separation, result contract); the gate page's long interface section replaced by a pointer; a Phase 49a Sparkplug "STALE quality" claim was corrected to verified birth/death liveness. Full gate green. **Slice D shipped** on `feat/phase51-ai-integration-model-families`: corpus note `model_families.md` + site page `/design/ai-integration/model-families/` (CNN/PINN/LLM as capability *and* poor fit — CNN train/test-leakage collapse + vibration-vs-motor-current, PINN loses-to-FEM/silent-failure/steady-state-reversion + the FP64/FP32 deployment-precision trap, LLM draft-then-verify with the agent kept out of the loop; dataset-licence note). **Source gaps closed:** FP64/PINN verified as Xu et al. NeurIPS 2025 (arXiv:2505.10949); LLM4PLC confirmed peer-reviewed (ICSE 2024 SEIP), Xia et al. peer-reviewed (IEEE ETFA 2025, DOI 10.1109/ETFA65518.2025.11205539) — both still POC. Register corrected: `llm_plc_code_draft` evidence_strength preprint→peer-reviewed; sources.yml expanded; data + mirror regenerated. Full gate green. **Slice E shipped** on `feat/phase51-ai-integration-digital-twin`: corpus note `digital_twin.md` + site page `/design/ai-integration/digital-twin/` (twin vs one-way data mirror; physical→digital vs digital→physical as separate problems; device-communication seam gated by the envelope; digital-to-physical ladder is the register's; CNN/PINN/LLM families inside the twin at their ceilings). **Research note re-verified before promotion:** the twin definition + four-domain reference model were grounded in **ISO 23247** (Digital Twin Framework for Manufacturing; verified via a NIST government publication citing the standard — ISO body not read), replacing the note's unsourced definition; `digital_twin_state_sync` row annotated + `iso_23247`/`nist_dt_manufacturing` added to sources. Full gate green. **Slice F shipped** on `feat/phase51-ai-integration-validation-lifecycle`: corpus note `validation_lifecycle.md` + site page `/design/ai-integration/validation-lifecycle/` (authority is perishable; lifecycle demands mapped to NIST AI RMF Govern/Map/Measure/Manage; hazard-derived `on_fail`/rollback failure response; component-wise test splits + uncertainty + deployment-condition validation). **NIST AI RMF Generative AI Profile (NIST AI 600-1) verified at publisher** (2024-07-26, companion to AI RMF 1.0) — closing the item deferred from 50.12; added as `nist_genai_profile` and cited on `llm_engineering_copilot`. **Model-evidence ledger promoted to a downloadable template** `docs/assets/templates/ai_model_evidence_ledger.md`, linked from a new "AI & Model Lifecycle" section on `/tools/templates/`. Full gate green. **Slice G shipped** on `feat/phase51-ai-integration-worked-architectures` — the final page: corpus note `worked_architectures.md` + site page `/design/ai-integration/worked-architectures/` (vision cell · predictive-maintenance pipeline · PINN soft sensor · read-only LLM copilot, each walked through the ladder in four beats — job, rung + why-not-higher, what stays independent, the gated contract). **Introduces no new technical claims** — composes Slices B–F. Full gate green. **THE SEVEN-PAGE AI-INTEGRATION EXPANSION IS COMPLETE and deployed** (all Review pending): gate · method-register · safety-boundaries · interfaces · model-families · digital-twin · validation-lifecycle · worked-architectures + the downloadable model-evidence ledger. **Outstanding follow-up:** author review of the eight Review-pending AI pages. Untouched by this build: Phase 50 Workstream D (site editorial — `review:` rollout for 166 legacy pages, IEC 62061 Ed 2.2 rebuild, vendor-link liveness), 50.6 thin CLI adapter + tests, owner sign-off on the 50.13 classification vocabulary → interactive selector, and Phase 49c (chem/bio authority, unauthorised). **Also queued:** owner sign-off on the 50.13 classification vocabulary → interactive selector; 50.6 thin CLI adapter + tests.

**Superseded Phase (completed as planned):** **Phase 49b — project hardening gate, then the AI/ML method register.** The accepted whole-project recommendation is `planning/2026-07-13-whole-project-architecture-recommendations.md`. **Before AI/ML implementation:** (P0.1) preserve I/O direction through the PLC module and fix the confirmed `YL-301` Modbus mapping defect; (P0.2) normalize the stale `UL 508A:2022` provenance across code, site, and corpus records; (P0.3) regenerate and verify the two-file RAG mirror drift. Then implement the governed release-check module/CI gate before the Phase 49b merge; the remaining table, validated-I/O, CLI/package, site-metadata, and state-cleanup recommendations may proceed as bounded hardening slices. **Existing AI/ML scope remains:** `docs/_data/ai-methods/*.yml` → rendered catalogue (~40–60 methods), with `max_authority`, `authority_basis`, `validation_required`, `failure_modes`, `safety_independence`, and `evidence_strength` on every row. **Still gated on:** (1) owner confirms the `/design/ai-integration/` placement; (2) the kHz/OPC UA argument is rewritten per `49a-findings.md` §2(a) before any interface page ships; (3) chemical/biological authority columns stay **Planned** until adversarial coverage is re-run. **Highest-value unblock:** purchase ISO/IEC TR 5469:2024; also check IEC 62061 for ML content. Spec: `docs/superpowers/specs/2026-07-12-ai-ml-methods-register-design.md`. Presentation plan: `planning/2026-07-12-ai-ml-control-systems-presentation-plan.md`. **Author actions outstanding:** review the two Phase 48 pages; re-review pages Phase 45 dropped to *Review pending*; settle the three UNVERIFIABLE purchased-text items; confirm provenance of the withheld legacy-panel photographs.
**Delivery Target:** GitHub Pages static site (content) + importable/installable Python package `cst` (tools), both personal-use first

**Phase 46.1 (2026-07-12) — COMPLETE.** A **fourth external review was found untriaged**: `prompt.md`, untracked at the repo root, never part of the `temp/` intake batch — and the upstream source of the assets Phase 46 shipped. Relocated to `planning/2026-07-12-design-page-review.md`. Three defects verified and fixed: the **lifecycle stage count** (`/design/` said 11, `/lifecycle/` said 13, the site publishes **14** — count dropped, not restated), the **false "through decommissioning" endpoint** (no such stage page exists; gap note added), the **duplicate "Motor Selection"** sidebar entry, and the residual **"Standards Atlas" branding** on live pages. Site identity is now *Control Systems Engineering Field Guide* throughout.

**AI/ML for Control Systems — Phase 49 PLANNED, build still gated.** Research moved to `control-standards/work/research/ai-ml-control-systems/` (work tier). Plan: `planning/2026-07-12-ai-ml-control-systems-presentation-plan.md`. Recommended placement is **one subsection under Design (`/design/ai-integration/`), not split across Fundamentals and Design** — co-location is a safety property, since a reader arriving at "CNNs" or "LLMs" from search must not meet the capability without the authority gate. Structure is **authority-first**: the research's own tree put safety 8th of 9; the ladder (offline → read-only → advisory → operator-approved → bounded supervisory → closed-loop) and the safety boundary become pages 1 and 2. **Owner decision outstanding: confirm or overrule the `/design/ai-integration/` placement.** Nothing is built until the source-register gaps close — above all *current standards/guidance for AI in machinery and functional safety* — and the two 2024 arXiv preprints are re-checked against the publisher.

**Phase 46 (2026-07-12) — COMPLETE.** Wire-colour-coding reference gallery live at `/design/wiring/wire-color-coding/` (15 diagrams, 8 groups) plus the design-package poster on `/tools/templates/` (pump & VFD skid across all ten deliverables, with the products-shown-are-examples caption). First image assets on the site; `.diagram-grid`/`.diagram-card` CSS remapped from the source README's MkDocs variables onto the site's design tokens. **One asset withheld:** the legacy-panel before/after graphic contains photographs of real panels whose provenance is unknown — the repo publishes original diagrams only. Alt text was written from the diagrams themselves; the shipped `assets.json` alt strings were auto-generated filename echoes. Also caught a Phase 45 straggler: the global-machine scenario still carried NFPA 79's stale "600 V max" scope.

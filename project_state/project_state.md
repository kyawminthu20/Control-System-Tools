...
**Next Phase:** **Phase 49b — the AI/ML method register** (`docs/_data/ai-methods/*.yml` → rendered catalogue; ~40–60 methods; the authority gate carried as *columns* on every row: `max_authority`, `authority_basis`, `validation_required`, `failure_modes`, `safety_independence`, `evidence_strength`). **Still gated on:** (1) owner confirms the `/design/ai-integration/` placement; (2) the kHz/OPC UA argument is rewritten per `49a-findings.md` §2(a) before any interface page ships; (3) chemical/biological authority columns stay **Planned** until adversarial coverage is re-run. **Highest-value unblock: purchase ISO/IEC TR 5469:2024** — its Usage-Level × Technology-Class matrix is the single most valuable missing artefact. Also outstanding: check **IEC 62061** for ML content. Spec: `docs/superpowers/specs/2026-07-12-ai-ml-methods-register-design.md`. **Superseded:** Phase 49's original 7-page topic plan (its authority-first principle is retained). Earlier **Phase 49 gating note**: nothing is built until (a) the owner confirms or overrules the recommended `/design/ai-integration/` placement, and (b) the source-register gaps close — above all *current standards/guidance for AI in machinery and functional safety* — with the two 2024 arXiv preprints re-checked against the publisher. Plan: `planning/2026-07-12-ai-ml-control-systems-presentation-plan.md`. See `governance/ROADMAP.md`. **Author actions outstanding:** (0) review the two Phase 48 pages (`/tools/manufacturers/vendor-documentation/`, `/tools/manufacturers/plc-hardware-families/`) — both at *Review pending*; the lifecycle statuses in particular are vendor claims captured July 2026 and were not independently verified; (1) re-review the pages Phase 45 dropped to *Review pending*; (2) settle the three UNVERIFIABLE items against purchased texts (IEC 60204-1 Cl. 9.2 e-stop categories; IEC 62061 SIL range; ISO 13849-1 Table 2 PL bands); (3) **confirm provenance of the legacy-panel photographs** in `temp/wire-color-coding-web-assets/14-legacy-panel-before-after.png` — withheld from publication in Phase 46 because the repo publishes only original diagrams, never third-party photos.
